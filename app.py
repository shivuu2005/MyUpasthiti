...
def admin_dashboard():
    ...
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM users")  # Assuming 'users' is the table storing student data
...
def user_view_attendance():
    if "user_id" in session:
        ...
        try:
            ...
            # Fetch attendance records for the last 10 days if the action is 'view_last_10_days'
            if action == "view_last_10_days":
                ten_days_ago = date.today() - timedelta(days=10)
                cur.execute("""
                    SELECT a.id, a.date, a.time, a.status, a.latitude, a.longitude, u.name, u.enrollment_no
                    FROM attendance a
                    JOIN users u ON a.user_id = u.id
                    WHERE a.user_id = %s AND a.date >= %s AND a.date <= %s
                    ORDER BY a.date DESC
                """, (session["user_id"], ten_days_ago, date.today()))
                attendance_records = cur.fetchall()
    ...
    if "user_id" in session:
        if request.method == "POST":
            ...
            if current_hour < 9 or current_hour > 24:
                flash("Attendance can only be marked between 9 AM and 11 AM.", "danger")
                return redirect(url_for("user_dashboard"))
